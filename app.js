...
if (page === "home") {
  ...
  // Live standings — now that the league is public, try fetching directly from
  // ESPN in the visitor's browser first (no cookies, no sync script needed).
  // Falls back to the cached data/standings.json (from the manual/CI sync) if
  // ESPN doesn't allow the cross-origin request.
  async function fetchLiveEspnStandings() {
    ...
    return { season: SEASON, leagueId: LEAGUE_ID, updatedAt: new Date().toISOString(), teams, live: true };
  }

  function renderStandings(data, isLive) {
    ...
    wrap.appendChild(el("p", {
      text: isLive
        ? `Live from ESPN · ${new Date(data.updatedAt).toLocaleString()}`
        : `Last synced ${new Date(data.updatedAt).toLocaleString()}`,
      attrs: { style: "color:var(--ink-soft); font-size:11.5px; margin-top:10px;" }
    }));
  }

  fetchLiveEspnStandings()
    .then(data => renderStandings(data, true))
    .catch(() => {
      // Fall back to whatever was last written by the manual/scheduled sync.
      fetch("data/standings.json")
        .then(res => (res.ok ? res.json() : null))
        .then(data => data && renderStandings(data, false))
